          ...
          <div v-if="settingsTab === 'package'">
            ...
            <div class="pts-pkg-grid">
              <div v-for="(p, i) in packagesList" :key="p.id || i" :class="['pts-pkg', { rec: p.rec }]">
                ...
                <div class="bprice">¥{{ currency((p.price || p.money || 0) / 100) }}<span v-if="p.originalPrice || p.old" style="font-size:12px;color:#9CA3AF;text-decoration:line-through;margin-left:6px;">{{ p.originalPrice || p.old }}</span><span v-if="p.save" class="bold">{{ p.save }}</span></div>
                <div class="bfooter">
...
function openEditPkg (p) { editing.type = 'pkg'; editing.id = p?.id ?? null; Object.assign(editForm, { name: p?.name || '', sub: p?.sub || '', pts: p?.pts || p?.points || 0, money: (p?.money || p?.price || 0) / 100, old: p?.old || p?.originalPrice || '', save: p?.save || '', rec: !!p?.rec, enabled: p?.enabled ?? true, rate: '', limit: '', cond: '' }); editModalVisible.value = true }
