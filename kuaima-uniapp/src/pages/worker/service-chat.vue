...
const messages = ref([]);
let wsBaseUrl = resolveWsBaseUrl();
...
function removeSocketListeners() {
  ...
  wsClient.off("MESSAGE", handleSocketMessage);
  wsClient.off("message", handleSocketMessage);
}
function connectIm() {
  removeSocketListeners();
  ...
  wsClient.on("MESSAGE", handleSocketMessage);
  wsClient.on("message", handleSocketMessage);
...
async function refreshMessages(initial = false) {
  ...
  try {
    ...
    else incoming.forEach(appendMessage);
    if (initial && incoming.length) scrollToLatest();
  } catch (_) {
    // WebSocket 可用时，历史同步失败不阻断实时聊天。
  }
}
...
function scrollToLatest() {
  // scroll-top 设置为一个足够大的值，scroll-view 会自动落到内容底部。
  // 先归零再设置，确保连续到达的消息每次都能触发滚动。
  scrollTop.value = 0;
  setTimeout(() => { scrollTop.value = 999999; }, 30);
}
function startMessageSync() {
  stopMessageSync();
  syncTimer = setInterval(() => refreshMessages(), 3000);
}
...
async function send() {
  ...
  try { const result = await sendChatMessage(sessionId.value, { fromId: Number(userId), content: text }); if (result) { messages.value.push(normalizeMessage(result)); scrollToLatest(); } } catch (error) { input.value = text; uni.showToast({ title: error.message || "发送失败", icon: "none" }); } finally { sending.value = false; }
}
function goBack() { uni.navigateBack(); }
</script>

<style scoped>
.page{height:100vh;display:flex;flex-direction:column;background:#f5f5f5;overflow:hidden}.chat-header{flex:none;display:flex;align-items:center;gap:18rpx;padding-left:20rpx;padding-right:20rpx;padding-bottom:22rpx;background:linear-gradient(135deg,#ff6b35,#ff8c5a);color:#fff}.header-action{width:56rpx;height:56rpx;display:flex;align-items:center;justify-content:center;font-size:50rpx;line-height:1}.header-action.more{font-size:28rpx;letter-spacing:2rpx}.header-avatar{width:80rpx;height:80rpx;display:flex;align-items:center;justify-content:center;flex-shrink:0;border-radius:50%;background:#fff;font-size:36rpx}.header-info{flex:1;min-width:0}.header-name{display:block;font-size:30rpx;font-weight:600}.header-status{display:flex;align-items:center;gap:8rpx;margin-top:6rpx;color:rgba(255,255,255,.85);font-size:22rpx}.status-dot{width:12rpx;height:12rpx;border-radius:50%;background:#52c41a}.security-tip{flex:none;display:flex;align-items:center;gap:12rpx;padding:16rpx 28rpx;border-bottom:1rpx solid #ffe4a0;background:#fff8e6;color:#b8860b;font-size:22rpx}.shield{color:#fa8c16;font-size:18rpx}.security-text{flex:1}.security-close{padding:0 8rpx;color:#fa8c16;font-size:34rpx}.chat-messages{flex:1;min-height:0;width:100%;height:0;padding:22rpx 28rpx 170rpx;box-sizing:border-box;background:#f5f5f5}.msg-time{display:block;margin:14rpx 0 24rpx;text-align:center;color:#999;font-size:20rpx}.msg-row{display:flex;align-items:flex-start;gap:16rpx;margin-bottom:28rpx}.msg-row.self{flex-direction:row-reverse}.msg-avatar{width:64rpx;height:64rpx;display:flex;align-items:center;justify-content:center;flex-shrink:0;border-radius:50%;color:#fff;font-size:25rpx}.msg-avatar.service{background:linear-gradient(135deg,#ff6b35,#ff8c5a)}.msg-avatar.user{background:#52c41a}.msg-content{max-width:75%}.hot-content{width:calc(100% - 80rpx);max-width:calc(100% - 80rpx)}.msg-bubble{padding:22rpx 26rpx;border-radius:8rpx 24rpx 24rpx;background:#fff;color:#333;font-size:26rpx;line-height:1.6;word-break:break-all}.self-bubble{border-radius:24rpx 8rpx 24rpx 24rpx;background:linear-gradient(135deg,#ff6b35,#ff8c5a);color:#fff}.message-time{display:block;margin-top:6rpx;color:#aaa;font-size:18rpx}.self .message-time{text-align:right}.hot-questions{padding:26rpx;border-radius:24rpx;background:#fff}.hot-title{display:block;margin-bottom:16rpx;color:#333;font-size:27rpx;font-weight:600}.hot-item{display:flex;align-items:flex-start;gap:12rpx;margin-top:14rpx;padding:18rpx 20rpx;border-radius:16rpx;background:#fff8f5;color:#ff6b35;font-size:23rpx;line-height:1.45}.hot-dot{padding-top:5rpx;font-size:12rpx}.hot-item.human{align-items:center;justify-content:center;background:#f5f5f5;color:#666}.scroll-space{height:30rpx}.composer{position:fixed;z-index:10;left:0;right:0;bottom:0;display:flex;align-items:center;gap:14rpx;padding:18rpx 24rpx 12rpx;border-top:1rpx solid #eee;background:#fff;box-sizing:border-box}.voice-button{width:64rpx;height:64rpx;display:flex;align-items:center;justify-content:center;color:#666;font-size:34rpx}.input-field{flex:1;height:76rpx;padding:0 26rpx;border-radius:40rpx;background:#f5f5f5;font-size:25rpx}.send-button{width:116rpx;height:76rpx;margin:0;padding:0;border:0;border-radius:40rpx;background:linear-gradient(135deg,#ff6b35,#ff8c5a);color:#fff;font-size:25rpx;line-height:76rpx}.send-button::after{border:0}.send-button[disabled]{opacity:.5}
</style>
