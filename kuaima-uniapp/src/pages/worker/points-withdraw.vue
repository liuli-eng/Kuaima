  ...
  <view class="page">
    ...
    <scroll-view scroll-y class="page-scroll">
      <view class="balance-card">
        ...
        <text class="balance-sub">约可提现 ¥{{ moneyValue }}</text>
        <view class="balance-rate"><image src="/static/icons/worker-points-withdraw/coins-white.svg" mode="aspectFit" />兑换比例 100积分 = 1元，提现免手续费</view>
      </view>
      ...
      <view class="form-card">
        ...
        >
          <view class="channel-icon" :class="channel.code">
            <image :src="channel.icon" mode="aspectFit" />
          </view>
          <view class="channel-info">
            <text class="channel-name">{{ channel.name }}</text>
            <text class="channel-desc">{{ channel.account }} · 预计T+1到账</text>
          </view>
      ...
      <view class="form-card">
        ...
        <view class="quick-row">
          ...
          <text class="quick-chip" :class="{ active: amount === balance }" @click="useAll">全部</text>
        </view>

        <view class="calc-block">
          <view class="calc-row highlight"><text>预计到账金额</text><text class="money">¥{{ moneyValue }}</text></view>
          <view class="calc-row"><text>兑换比例</text><text>100积分 = 1元</text></view>
          <view class="calc-row"><text>提现积分</text><text>{{ formatPoints(amount) }} 积分</text></view>
          <view class="calc-row"><view class="fee-label"><text>手续费</text><image src="/static/icons/worker-points-withdraw/circle-info-gray.svg" mode="aspectFit" /></view><text class="free">¥0.00（限时免费）</text></view>
        </view>
      </view>

      <view class="rule-card">
        <view class="rule-title"><image src="/static/icons/worker-points-withdraw/lightbulb-orange.svg" mode="aspectFit" />提现规则</view>
        <view class="rule-item"><image src="/static/icons/worker-points-withdraw/circle-peach.svg" mode="aspectFit" /><text>最低 1,000 积分（¥10）起提，提现积分须为 100 的整数倍；</text></view>
        <view class="rule-item"><image src="/static/icons/worker-points-withdraw/circle-peach.svg" mode="aspectFit" /><text>兑换比例 100积分=1元，当前限时免收手续费；</text></view>
        ...
        <view class="rule-item"><image src="/static/icons/worker-points-withdraw/circle-peach.svg" mode="aspectFit" /><text>每日最多提现 1 次；提现失败时积分将原路退回，可在提现记录查看。</text></view>
      </view>
      <view class="bottom-space" />
    </scroll-view>

    <SafeBottomAction>
      <button class="submit-btn" :class="{ disabled: !canSubmit }" @click="openConfirm">立即提现</button>
    </SafeBottomAction>
    ...
    <view v-if="confirmVisible" class="confirm-sheet">
      ...
      <view class="confirm-row"><text>到账方式</text><text>{{ selectedChannelItem.name }}（{{ selectedChannelItem.account }}）</text></view>
      <view class="confirm-row"><text>手续费</text><text class="free">¥0.00</text></view>
    ...
    <view v-if="successVisible" class="success-sheet">
      <view class="success-wrap">
        ...
        <text class="success-desc">积分已冻结，预计 <text class="success-orange">明日24:00</text> 前到账<br />到账结果将通过站内消息通知你</text>
        <view class="success-card">
          <view class="confirm-row"><text>提现积分</text><text>{{ formatPoints(amount) }} 积分</text></view>
          <view class="confirm-row"><text>到账金额</text><text class="success-orange">¥{{ moneyValue }}</text></view>
          <view class="confirm-row"><text>到账方式</text><text>{{ selectedChannelItem.name }}</text></view>
          <view class="confirm-row"><text>申请单号</text><text>WDJF{{ orderNo }}</text></view>
        </view>
        <view class="sheet-actions">
...
const orderNo = ref("");
const channels = [
  { code: "wechat", icon: "/static/icons/worker-points-withdraw/weixin-white.svg", name: "微信零钱", account: "张建国（已实名）" },
  { code: "alipay", icon: "/static/icons/worker-points-withdraw/alipay-white.svg", name: "支付宝", account: "138****8562" },
];
const quickValues = [1000, 2000, 3000];
const amount = computed(() => Number(String(amountInput.value).replace(/[^\d]/g, "")) || 0);
const moneyValue = computed(() => (amount.value / 100).toFixed(2));
const canSubmit = computed(() => amount.value >= 1000 && amount.value <= balance.value && amount.value % 100 === 0);
const selectedChannelItem = computed(() => channels.find((item) => item.code === selectedChannel.value) || channels[0]);
...
if (userId) {
  getPoints(userId)
    .then((result) => {
      balance.value = Number(result?.balance ?? result?.points ?? 0);
    })
    .catch(() => {});
}
...
function pickAmount(value) {
  amountInput.value = String(Math.min(value, balance.value));
}
function useAll() {
  amountInput.value = String(balance.value);
}
...
function openConfirm() {
  if (!canSubmit.value) {
    uni.showToast({ title: amount.value ? "最低1000积分且须为100的整数倍" : "请输入提现积分", icon: "none" });
    return;
  }
...
function submitWithdraw() {
  ...
  setTimeout(() => {
    ...
    orderNo.value = `${new Date().getFullYear()}${String(Date.now()).slice(-8)}`;
