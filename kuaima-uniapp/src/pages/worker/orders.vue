      ...
      <view v-for="order in filtered" :key="order.id" class="card">
        ...
        <view class="footer"
          ...
          ><view class="actions"
            ...
            <button v-if="order.status === 'hired'" class="cancel" @click="updateStatus(order, 'work')">确认到岗</button>
            <button v-if="order.status === 'arrived'" class="cancel" @click="updateStatus(order, 'finish')">确认完工</button></view
          ></view
        >
      </view>
...
const dates = [
  ...
  { key: "yesterday", label: "昨天", sub: "8/19" },
  { key: "today", label: "今天", sub: "8/20" },
  { key: "tomorrow", label: "明天", sub: "8/21" },
  { key: "after", label: "后天", sub: "8/22" },
];
const statuses = [
  { key: "all", label: "全部状态" },
...
const filtered = computed(() =>
  mock.value.filter(
    (i) =>
      (orderType.value === "day" ? i.group === "day" : true) &&
...
onMounted(async () => {
  try {
    ...
    if (Array.isArray(items)) {
      mock.value = await Promise.all(items.map(async (item) => {
        try {
          const order = await request({ url: `/worker/jobs/${item.orderId}` });
          return normalizeOrder({ ...order, ...item });
        } catch (_) {
...
function open(o) {
  uni.navigateTo({ url: `/pages/worker/order-detail?id=${o.id}` });
}
function cancel(o) {
  uni.showModal({
    title: "取消报名",
...
function normalizeOrder(item) {
  const statusMap = { 已报名: "applied", 已录用: "hired", 已到岗: "arrived", 已完成: "done", 取消招工: "cancelled", 取消报名: "cancelled" };
  return { ...item, title: item.orderTitle || item.title || `订单 ${item.orderId || item.id}`, type: item.type === "month" ? "月结" : item.type === "heldBack" ? "压薪日结" : "日结", typeClass: item.type === "month" ? "monthly" : item.type === "heldBack" ? "press" : "day", group: item.type === "month" ? "monthly" : item.type === "heldBack" ? "press" : "day", tags: [], employer: item.bossName || item.companyName || "平台认证雇主", time: [item.startTime, item.endTime].filter(Boolean).join("-") || item.date || "", address: item.address || item.workAddress || "", distance: item.distance || "", amount: item.wage ? (Number(item.wage) / 100).toFixed(2) : (item.salary || 0), unit: item.type === "month" ? "元/月" : "元/天", status: statusMap[item.status] || "applied", statusText: item.status || "已报名", date: "all" };
}
</script>
<style scoped>
.page {
