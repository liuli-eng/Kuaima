  ...
  <view class="container">
    <scroll-view scroll-y class="scroll-area">
      ...
      <view class="notice-bar">
        ...
        <text class="notice-text">通话将被录音，禁止交换电话号码和微信；零工完工后...</text>
      </view>
      ...
      <!-- 实名认证Banner -->
      <view class="cert-bar" @click="navigateTo('realname')">
        ...
        <view class="job-card" v-for="job in filteredJobs" :key="job.id">
          ...
          <view class="job-actions">
            <template v-if="job.status === 'recruiting'">
              <text class="job-btn btn-primary" @click="navigateTo('applicant-info', { orderId: job.id })">查看报名</text>
              <text class="job-btn btn-secondary" @click="navigateTo('publish', { id: job.id })">编辑招工</text>
              <text class="job-btn btn-danger" @click="cancelJob(job.id)">取消招工</text>
            </template>
            <template v-else-if="job.status === 'ended'">
              <text class="job-btn btn-secondary" @click="confirmArrival(job.id)">确认到岗</text>
              <text class="job-btn btn-link" @click="navigateTo('publish', { id: job.id, readonly: 1 })">详情</text>
            </template>
            <template v-else-if="job.status === 'settling'">
              <text class="job-btn btn-primary" @click="navigateTo('suspend-settle', { orderId: job.id })">去结算</text>
              <text class="job-btn btn-link" @click="navigateTo('publish', { id: job.id, readonly: 1 })">详情</text>
            </template>
    ...
    <view class="modal-mask" v-if="showCancelModal" @click="showCancelModal = false">
      <view class="modal-content" @click.stop>
        ...
        <text class="modal-desc">取消后将不再接收零工报名，已报名的零工将收到通知</text>
        <view class="modal-actions">
          <text class="modal-btn modal-btn-cancel" @click="showCancelModal = false">再想想</text>
          <text class="modal-btn modal-btn-confirm" @click="confirmCancel">确认取消</text>
        </view>
      </view>
    </view>

    <!-- 确认到岗弹窗 -->
    <view class="modal-mask" v-if="showConfirmModal" @click="showConfirmModal = false">
      <view class="modal-content" @click.stop>
        <text class="modal-title">确认全部零工已到岗？</text>
        <text class="modal-subtitle">{{ confirmTargetTitle }}</text>
        <text class="modal-desc">确认后状态将变更为待结算，可前往结算页面进行付款</text>
        <view class="modal-actions">
          <text class="modal-btn modal-btn-cancel" @click="showConfirmModal = false">再想想</text>
          <text class="modal-btn modal-btn-confirm" @click="confirmArrive">确认到岗</text>
        </view>
      </view>
    </view>
  </view>
...
const demoJobs = [
  { id: 1, title: '电商分拣打包', workTime: '08:00 ~ 18:00', location: '松江区车墩镇', wage: '180元/天', recruitCount: 5, currentApply: 13, status: 'recruiting' },
  { id: 2, title: '餐饮服务员', workTime: '10:00 ~ 20:00', location: '松江区中山街道', wage: '150元/天', recruitCount: 2, currentApply: 5, status: 'ended' },
  { id: 3, title: '快递搬运装卸工', workTime: '07:00 ~ 17:00', location: '松江区九亭镇', wage: '200元/天', recruitCount: 4, currentApply: 8, status: 'settling' },
  { id: 4, title: '冷库分拣员', workTime: '06:00 ~ 16:00', location: '松江区泗泾镇', wage: '220元/天', recruitCount: 3, currentApply: 6, status: 'completed' },
  { id: 5, title: '装配工', workTime: '08:00 ~ 18:00', location: '松江区新桥镇', wage: '250元/天', recruitCount: 5, currentApply: 11, status: 'cancelled' },
  { id: 6, title: '超市理货员', workTime: '09:00 ~ 19:00', location: '松江区洞泾镇', wage: '160元/天', recruitCount: 2, currentApply: 4, status: 'recruiting' }
]
...
function normalizeOrder(order, currentApply = 0) {
  return {
    ...
    wage: `${Number(order.salary || 0)}元/天`,
...
export default {
  ...
  computed: {
    filteredJobs() {
      ...
      return this.jobList.filter(j => j.status === this.currentFilter)
    }
  },
  methods: {
    navigateTo(pageName, params = {}) {
      const bossPages = [
        ...
        'boss-publish', 'publish', 'search-worker', 'select-job', 'publish-info', 'schedule-stats', 
        ...
        'signup-notice', 'invite-friend', 'service-chat', 'insurance', 'realname', 'applicant-info'
      ]
    ...
    async loadOrders() {
      ...
      try {
        ...
        if (orders.length > 0) {
          this.jobList = await Promise.all(orders.map(async (order) => {
            try {
              const items = await listOrderItems(order.id)
              return normalizeOrder(order, Array.isArray(items) ? items.length : 0)
            } catch (_) {
        ...
        uni.showToast({ title: error.message || '招工列表加载失败', icon: 'none' })
      } finally {
        this.loading = false
      }
    },
...
.status-recruiting { background: #FFF3E0; color: #FF6B35; }
.status-ended { background: #E6F7FF; color: #1890FF; }
.status-settling { background: #FFF8E6; color: #FA8C16; }
